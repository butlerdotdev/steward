...
While Steward innovates in how control planes are managed, Tenant Worker Nodes remain true to their Kubernetes roots: they are regular virtual machines or bare metal servers that run your workloads. What makes them special in Steward's architecture is how they integrate with the containerized control planes and how they can be managed at scale across diverse infrastructure environments.

## Understanding Worker Nodes in Steward

In a Steward managed cluster, worker nodes connect to their Tenant Control Plane just as they would in a traditional Kubernetes setup. The key difference is that the control plane they're connecting to runs as pods within the Management Cluster, rather than on dedicated machines. This architectural choice maintains compatibility with existing tools and workflows while enabling more efficient resource utilization.

Each worker node belongs to exactly one Tenant Cluster and runs only that tenant's workloads. This clear separation ensures strong isolation between different tenants' applications and data, making Steward suitable for multi-tenant environments.
...
Steward supports multiple approaches to managing worker node lifecycles:
...
!!! tip "yaki"
    See [yaki](https://goyaki.butlerlabs.io/) script, which you could modify for your preferred operating system and version. The provided script is just a facility: it assumes all worker nodes are running `Ubuntu`. Make sure to adapt the script if you're using a different OS distribution.
...
For more sophisticated automation, Steward provides a [Cluster API Control Plane Provider](https://github.com/butlerlabs/cluster-api-control-plane-provider-steward).
...
Steward's approach to worker nodes combines the familiarity of traditional Kubernetes with the flexibility to run anywhere and the ability to manage at scale. Whether you're building a private cloud platform, offering Kubernetes as a service, or managing edge computing infrastructure, Steward provides the tools and patterns you need.
