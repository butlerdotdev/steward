...
Steward offers the possibility of having a different storage system than `etcd` thanks to [kine](https://github.com/k3s-io/kine) integration.
...
!!! warning "Not for production"
    The default settings are not production grade: the following scripts are just used to test the Steward usage of different drivers.

## Defining a default Datastore upon Steward installation

Use Helm to install the Steward Operator and make sure it uses a datastore with the proper driver `datastore.driver=<MySQL|PostgreSQL|NATS>`.
...
```bash
helm install steward charts/steward -n steward-system --create-namespace \
  --set steward-etcd.deploy=false \
  --set datastore.driver=PostgreSQL \
  --set datastore.endpoints[0]=postgres-default-rw.steward-system.svc:5432 \
  --set datastore.basicAuth.usernameSecret.name=postgres-default-superuser \
  --set datastore.basicAuth.usernameSecret.namespace=steward-system \
  ...
  --set datastore.basicAuth.passwordSecret.namespace=steward-system \
  ...
  --set datastore.tlsConfig.certificateAuthority.certificate.namespace=steward-system \
  ...
  --set datastore.tlsConfig.certificateAuthority.privateKey.namespace=steward-system \
  ...
  --set datastore.tlsConfig.clientCertificate.certificate.namespace=steward-system \
  ...
  --set datastore.tlsConfig.clientCertificate.privateKey.namespace=steward-system \
...
When the said key is omitted, Steward will use the default datastore configured with its CLI argument `--datastore`.
