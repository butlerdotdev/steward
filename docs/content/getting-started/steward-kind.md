# Steward on Kind
This guide will lead you through the process of creating a working Steward setup using Kind cluster. The guide requires the following installed on your workstation: `docker`, `kind`, `helm`, and `kubectl`.

!!! warning "Development Only"
    Run Steward on kind only for development or learning purposes.
    
    Steward is designed to be run on production-grade Kubernetes clusters, such as those provided by cloud providers or on-premises solutions. Kind is not a production-grade Kubernetes cluster, and it is not recommended to run in production environments.
...
## Summary
  ...
  * [Installing Steward](#installing-steward)
...
Steward has a dependency on Cert Manager, as it uses dynamic admission control, validating and mutating webhook configurations which are secured by a TLS communication, these certificates are managed by `cert-manager`. Hence, it needs to be added. 
...
## Installing Steward
- Add the Butler Labs Repo to the Helm Manager.

```
helm repo add butlerlabs https://butlerlabs.github.io/charts
```

- Install Steward with Helm

```
helm upgrade --install steward butlerlabs/steward \
...
- Verify by first checking Steward CRDs

```
kubectl get crds | grep -i steward
```

!!! Info "CSI Drivers"
    Steward requires a __storage provider__ installed on the management cluster. Kind by default provides `local-path-provisioner`, but one can have any other CSI Drivers.
...
```
kubectl apply -f https://raw.githubusercontent.com/butlerlabs/steward/master/config/samples/steward_v1alpha1_tenantcontrolplane.yaml
```
...
SECRET=""
kubectl get secret $SECRET -o jsonpath='{.data.admin\.conf}'|base64 -d > /tmp/steward.conf
```
...
```
docker exec -it $(docker container list | grep steward-control-plane | awk '{print $1}') bash
```

- Export the `kubeconfig` file to the environment variable `KUBECONFIG`

```
export KUBECONFIG=/tmp/steward.conf
```
