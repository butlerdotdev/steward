# Steward on generic infra
This guide will lead you through the process of creating a working Steward setup on a generic infrastructure.
...
## Summary
  ...
  * [Install Steward controller](#install-steward-controller)
...
```bash
git clone https://github.com/butlerlabs/steward
cd steward/deploy
```
...
In Steward, the Management Cluster is a regular Kubernetes cluster which hosts zero to many Tenant Cluster Control Planes. The Management Cluster acts as cockpit for all the Tenant Clusters as it hosts monitoring, logging, and governance of Steward setup, including all Tenant Clusters. 
...
Any regular and conformant Kubernetes v1.22+ cluster can be turned into a Steward setup. To work properly, the Management Cluster should provide:
...
Make sure you have a `kubeconfig` file with admin permissions on the cluster you want to turn into Steward Management Cluster and check you can access:
...
## Install Steward Controller

Installing Steward via Helm charts is the preferred way to deploy the Steward controller.
The Helm chart is available in the `charts` directory of the Steward repository, or as Helm Chart versioned as `0.0.0+latest`

!!! info "Stable Releases"
    As of July 2024 [Butler Labs Labs](https://github.com/butlerlabs) no longer publish version pinned release artifacts.
    Version pinned and stable releases are offered on a subscription basis by [BUTLERLABS](https://butlerlabs.io), the main Steward project contributor. 

Run the following commands to install the latest edge release of Steward:

```bash
helm install steward butlerlabs/steward \
...
!!! note "Alternative: Two-Step Installation"
    The steward chart includes CRDs. For separate CRD management (GitOps, policy requirements), install CRDs first

```bash
# Step 1: Install CRDs
helm install steward-crds butlerlabs/steward-crds -n steward-system --create-namespace --version 0.0.0+latest

# Step 2: Install Steward operator
helm install steward butlerlabs/steward -n steward-system --version 0.0.0+latest
```

After installation, verify that Steward and its components are running:
...
steward-etcd-0                1/1     Running     0             50s
steward-etcd-1                1/1     Running     0             60s
steward-etcd-2                1/1     Running     0             90s
steward-7949578bfb-lj44p      1/1     Running     0             12s
```

!!! info "Steward Datastore"
    Steward installs [steward-etcd](https://github.com/butlerlabs/steward-etcd) as its default datastore, which is a multi-tenant `etcd`. Optionally, Steward offers support for other storage systems, as `PostgreSQL`, `MySQL` or `NATS`, thanks to the native [kine](https://github.com/k3s-io/kine) integration.
...
metadata:
  ...
  labels:
    tenant.butlerlabs.io: ${TENANT_NAME}
spec:
  dataStore: default
  controlPlane:
    deployment:
...
metadata:
  name: sample-tcp
  labels:
    tenant.butlerlabs.io: sample-tcp
spec:
  controlPlane:
    deployment:
      replicas: 2
...
The Tenant Control Plane is made of pods running in the Steward Management Cluster. At this point, the Tenant Cluster has no worker nodes. So, the next step is to join some worker nodes to the Tenant Control Plane.
...
Delete the tenant control plane from Steward
...
### Uninstall Steward
Uninstall the Steward controller by removing the Helm release
...
```bash
kubectl -n steward-system delete pvc --all
```

Also delete the custom resources:

```bash
kubectl delete crd tenantcontrolplanes.steward.butlerlabs.io
kubectl delete crd datastores.steward.butlerlabs.io
```

In case of a broken installation, manually remove the hooks installed by Steward:

```bash
kubectl delete ValidatingWebhookConfiguration steward-validating-webhook-configuration
kubectl delete MutatingWebhookConfiguration steward-mutating-webhook-configuration
```
